...
let guestList:string[] = ['Sikandar', 'Auranzaib', 'Muneeb',];
for(let i=0; i<guestList.length; i++){
    console.log(`Dear ${guestList[i]}, you are invited to dinner!`);
}

 
console.log(`\nMr ${guestList[1]} could not make it\n`);
...
for(let j=guestList.length; j=guestList.length-2; j--){
    console.log(`Sorry Mr. ${guestList[j]}, You're Not invited for tonight dinner due to tabble issue `);
    guestList.pop()
}

console.log(`\n Mr. ${guestList[0]}, you are still invited for annual dinner`);
console.log(`Mr. ${guestList[1]}, you are still invited for annual dinner`);
guestList.pop();
guestList.pop();
